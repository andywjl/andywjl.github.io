import { getAllProjects, getProject, getCampus, getAllIssues } from "@/lib/data";
import { notFound } from "next/navigation";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { StatusBadge, RiskBadge } from "@/components/alert-badge";
import { FolderKanban, Target, Calendar, DollarSign, Clock, CheckCircle } from "lucide-react";
...
export default async function ProjectDetailPage({
  ...
  const campus = getCampus(project.campusId);
  const linkedIssues = getAllIssues().filter(i => i.projectId === project.id);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <Card>
          ...
          <CardContent><p className="text-sm text-gray-700">{project.expectedEffect}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-600" />实际效果</CardTitle></CardHeader>
          <CardContent><p className="text-sm text-gray-700">{project.actualEffect || "待验证"}</p></CardContent>
        </Card>
      </div>

      {linkedIssues.length > 0 && (
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">关联问题 ({linkedIssues.length})</CardTitle></CardHeader>
